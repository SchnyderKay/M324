...
The logger in this application is implemented using Log4j2 and is designed to capture important events and metrics throughout the API's lifecycle. It is primarily used to log incoming requests, responses, and any errors that occur during processing.

## Key Features

1. **Logging Incoming Requests**:
- Each API call is logged with its HTTP method, request URI, parameters, headers, and payload. This information helps in tracking the flow of requests through the system.

2. **Logging Successful Responses**:
- Successful operations (e.g., retrieval, creation, updates) are logged with a success message, indicating the operation type and the corresponding resource identifier.

3. **Logging Errors**:
- When errors occur, such as resource not found or conflicts, detailed error messages are logged. This includes information about the request that caused the error, making it easier to debug issues.

4. **Exception Handling**:
- Specific exceptions (e.g., `DrinkNotFoundException`, `DrinkAlreadyExistsException`) are caught and logged with error messages. This allows for better tracking of recurring issues.

## Metrics Provided

The logger allows for the collection of several key metrics, including:

- **Total Number of Requests**: Count of all incoming requests logged.
- **Successful Requests**: Number of requests that were successfully processed (HTTP 200 or 201 status).
- **Failed Requests**: Count of requests that resulted in errors (e.g., HTTP 404, 500).
- **Request Distribution by Method**: Breakdown of requests by HTTP method (GET, POST, PUT, DELETE).
- **Frequency of Specific Errors**: Tracking of how often certain errors occur, which helps identify potential issues in the application.
- **Endpoint Usage**: Insights into which API endpoints are most frequently accessed.

## Log Storage and Access

The logs are stored in a specified log file as defined in the Log4j2 configuration (`log4j2.properties`). The log files can be accessed by the development and operations teams for monitoring and troubleshooting purposes. These logs provide valuable insights into the application's performance, usage patterns, and areas that may require optimization.

## Current Limitations

Due to some issues that could not be fixed in time, the logger currently cannot upload logs to AWS. In order to prevent the backend from breaking due to this error, the following arrangements have been made in the branch `<77-logger>`:

- The code for the logger has been added, including a fully configured `log4j2.properties` file that sets up the environment for logging.
- Dependencies for AWS CloudWatch have been included in the Gradle build file.
- Each Controller class has been updated to include `log.info` and `log.error` statements to ensure that logs are recorded during request processing.

## How It Would Work

The logging system is designed to seamlessly integrate with AWS CloudWatch, enabling efficient monitoring and storage of logs. Here’s how it should work:

1. **Logging Mechanism**: When the application is running correctly, logs generated by the logger would be automatically sent to AWS CloudWatch. Each log entry would contain relevant information about requests and responses.

2. **Log Groups and Streams**: The logs would be organized into specific log groups in CloudWatch, allowing for easy categorization and management. Each log group would have a defined name, and within that group, there would be individual log streams.

3. **Real-time Monitoring**: The logs would appear in the corresponding log stream within the defined log group, providing real-time insights into application behavior. Each log entry would be timestamped, allowing developers and operations teams to monitor the application's performance and troubleshoot issues effectively.

4. **Access and Analysis**: The logs stored in CloudWatch would be accessible for further analysis, making it easy to identify trends, diagnose problems, and ensure that the application operates smoothly.

This integration with CloudWatch would greatly enhance the monitoring capabilities of the application, ensuring that all relevant logs are captured and accessible for troubleshooting and performance tuning.
